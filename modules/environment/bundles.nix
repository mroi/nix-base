{ config, lib, pkgs, ... }: {
	...
	config = let
		...
		bundleScript = mode: path: attrs: lib.optionalString (mode == "update") ''
			...
			if test -r '${path}/Contents/Info.plist' ; then
				version=$(xmllint --xpath '/plist/dict/key[text()="CFBundleShortVersionString"]/following-sibling::string[1]/text()' '${path}/Contents/Info.plist')
			else
				version=
			fi
	...
	in lib.mkIf (config.environment.bundles != {}) {
		...
		system.updateScripts.bundles = ''
			storeHeading 'Updating side-loaded bundles'
