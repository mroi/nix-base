{ config, lib, pkgs, ... }: {
	...
	options.security.pki.certificateTrust = let
		...
		trustType = lib.types.nullOr (lib.types.attrsOf (lib.types.submodule { options = {
			...
			private = lib.mkEnableOption "private root certificate";
	...
	config = let
		...
		'' + lib.optionalString (mode == "system") ''
			...
			private='${lib.concatLines (lib.attrNames (lib.filterAttrs (n: v: v.private) config.security.pki.certificateTrust."${mode}"))}'
			for hash in $config ; do
				if ! hasLine "$systemRoots" "$hash" && ! hasLine "$private" "$hash" ; then
					printWarning 'Unknown system root certificate'
