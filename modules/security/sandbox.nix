{ config, lib, pkgs, ... }: {
	...
	config = lib.mkIf config.security.sandbox.enable {
		...
		system.build.packages.sandbox = let
			...
			isInstalled = name: lib.any (lib.hasSuffix "#${name}") config.environment.profile;
