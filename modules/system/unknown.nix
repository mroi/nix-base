{ config, lib, pkgs, ... }: {
	...
	in lib.mkIf condition {
		...
		system.cleanupScripts.unknown = lib.stringAfter [ "files" ] ''
			...
			# delete unknown unprotected files interactively
			echo 'SELECT path FROM files WHERE known IS FALSE AND protected IS FALSE ORDER BY path;' | runSQL | \
			...
			# delete unknown protected files in recovery
			echo 'SELECT path FROM files WHERE known IS FALSE AND protected IS TRUE ORDER BY path;' | runSQL | \
